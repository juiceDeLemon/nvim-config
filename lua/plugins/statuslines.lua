local function heirline()
	...
	local modes = {
		...
		provider = function(self)
			return "ඞ " .. self.mode_names[vim.fn.mode(1)] .. " "
		end,
		hl = function(self)
			local colour = self:mode_colour()
	...
	-- one_space
	local one_space = { provider = " ", hl = { bg = "bg" } }
	...
	local file_pathname = {
		provider = function(self)
			...
			if not cond.width_percent_below(#filename, 0.2) then
	...
	file_name = utils.insert(file_name, file_pathname, file_flags)
	...
	local venv = {
		condition = function()
			return vim.fn.environ()["VIRTUAL_ENV"] ~= nil
		end,
		provider = function()
			return vim.fn.environ()["VIRTUAL_ENV_PROMPT"]
		end,
	...
	local snippet_indicator = {
		condition = function()
			return snip.jumpable(-1) or snip.jumpable(1) or snip.choice_active()
		end,
		provider = function()
			local backward = snip.jumpable(-1) and "<-" or ""
			local choice = snip.choice_active() and "!" or " "
			local foward = snip.jumpable(1) and "->" or ""
			return "[" .. backward .. choice .. foward .. "]"
		end,
		update = { "CursorMovedI", "ModeChanged" },
		hl = mhl,
	}
	...
	local harpoon = {
		...
		provider = "[harpoon]",
		update = { "BufReadPost", "UiEnter", "ModeChanged" },
		hl = mhl,
	}
	...
	local encoding = {
		init = function(self)
			self.encoding = vim.opt.fileencoding:get()
		end,
		provider = function(self)
	...
	local bufaltflag = {
		condition = function(self)
			...
			if (thing == vim.fn.expand "#") and (thing ~= "") then
				return true
			end
		end,
		provider = "#",
